    ...
    while True :
        ...
        if isLongTake == False and isShortTake == False and TradeDateStop != DataCrypto['date'][srcCloseElement]:
            ...
            if arrow[srcCloseElement] == 1 and color_bar[srcCloseElement] == 1 and lastRedID >= 0: #or color_bar[srcCloseElement] == 0):
                ...
                TakeProfitPriceMult = 0.0
                if lastNumberGreen >= 3:
                    StopLossPrice = DataCrypto['HA_open'][srcCloseElement-2]
                    TakeProfitPriceMult = 2.5
                else:
                    StopLossPrice = DataCrypto['HA_low'][lastRedID]
                    TakeProfitPriceMult = 1.0
                if float(DataCrypto['HA_open'][srcCloseElement]) - ((float(DataCrypto['HA_open'][srcCloseElement]) * 3.0)/100) <= float(StopLossPrice) and StopLossPrice < DataCrypto['HA_open'][srcCloseElement]:

                    StopLossPrice = round_decimals_down(StopLossPrice, int(pricePrecision))
                    TakeProfitPrice =  DataCrypto['open'][srcCloseElement] + (((DataCrypto['close'][lastRedID] - DataCrypto['open'][srcCloseElement]) * TakeProfitPriceMult) * -1.0)
                ...
                if float(DataCrypto['HA_open'][srcCloseElement]) + ((float(DataCrypto['HA_open'][srcCloseElement]) * 3.0)/100) >= float(StopLossPrice) and StopLossPrice > DataCrypto['HA_open'][srcCloseElement]:
                    ...
                    StopLossPrice = round_decimals_down(StopLossPrice, int(pricePrecision))
                    TakeProfitPrice =  DataCrypto['open'][srcCloseElement] - (((DataCrypto['close'][lastGreenID] - DataCrypto['open'][srcCloseElement]) * TakeProfitPriceMult))
                    TakeProfitPrice = round_decimals_down(TakeProfitPrice, int(pricePrecision))
                    while True:
                        try:
