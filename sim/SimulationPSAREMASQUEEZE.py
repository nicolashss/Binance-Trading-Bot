...
if __name__ == "__main__":
    ...
    for i in range(len(DataCrypto['close'])):
        ...
        if isLongTake == True :
            ...
            if lastBullSAR != -1 and lastBearSAR != -1 and SAR['psarbull'][i-1] != None and (SAR['psarbear'][i-2] != None or SAR['psarbear'][i-3] != None) and SAR['psarbull'][lastBearSARIndex+1] > EMA_100[i-1] and lastBearSAR > EMA_100[i-1] and DataCrypto['close'][i-1] > lastBearSAR and (colors[i-1] == 'maroon'):
                ...
                if float(DataCrypto['open'][i]) - ((float(DataCrypto['open'][i]) * 3.0)/100) <= float(DataCrypto['low'][lastBearSARIndex]) and StopLossPrice < DataCrypto['open'][i]:
                    ...
                    TakeProfitPrice =  DataCrypto['open'][i] + (((DataCrypto['low'][lastBearSARIndex] - DataCrypto['open'][i]) * 10.0) * -1.0)
            ...
            elif lastBullSAR != -1 and lastBearSAR != -1 and SAR['psarbear'][i-1] != None and (SAR['psarbull'][i-2] != None or SAR['psarbull'][i-3] != None) and SAR['psarbear'][lastBullSARIndex+1] < EMA_100[i-1] and lastBullSAR < EMA_100[i-1] and DataCrypto['close'][i-1] < lastBullSAR and (colors[i-1] == 'green') and lastBullSARIndex - i < 2:
                ...
                if float(DataCrypto['open'][i]) + ((float(DataCrypto['open'][i]) * 3.0)/100) >= float(DataCrypto['high'][lastBullSARIndex]) and StopLossPrice > DataCrypto['open'][i]:
                    ...
                    TakeProfitPrice =  DataCrypto['open'][i] - (((DataCrypto['high'][lastBullSARIndex] - DataCrypto['open'][i]) * 10.0))
                    stoplossArray[i] = StopLossPrice
                    TakeProfitArray[i] = TakeProfitPrice
