...
def getUSDTBalanceSTR():
    ...
    try:
        ...
        print(e)
        telegram_bot.sendMessage(chat_id, "[BOT] : Erreur : Je n'arrive pas à récupérer le montant de ta balance BUSD, peut-être un problème d'api, je m'arrête.")
        os.execv(sys.executable, [sys.executable, __file__] + sys.argv)
    
    for i in range(len(acc_balance)) :
        if acc_balance[i]['asset'] == 'BUSD':
...
def SSL_Custom(df):
    ...
    for i in range(len(color_bar)):
        if(df['close'][i] > upperk[i]):        
            color_bar[i] = 1
        elif(df['close'][i] < lowerk[i]):
            color_bar[i] = -1
        else:
            color_bar[i] = 0

    ExitHigh = HMA(df['high'], 15)
    ExitLow = HMA(df['low'], 15)
    ...
    for i in range(len(ExitHigh)):
        ...
        if(df['close'][i] > ExitHigh[i]):
            Hlv3 = -1
        elif(df['close'][i] < ExitLow[i]):
            Hlv3 = 1
        else:
            Hlv3 = lastHlv3
        ...
        if Hlv3 < 0:
            if df['close'][i] > ExitHigh[i] and df['close'][i-1] < ExitHigh[i-1]:
                arrow[i] = 1
            else:
                arrow[i] = 0
        else:
            if ExitLow[i] > df['close'][i] and ExitLow[i-1] < df['close'][i-1]:
                arrow[i] = -1
            else:
                arrow[i] = 0
...
if __name__ == "__main__":
    ...
    print(actualBalance)
    message = "[BOT] : Voici ta balance : " + str(actualBalance) + " BUSD"
    telegram_bot.sendMessage(chat_id, message)
    ...
    while True :
                ...
                if arrow[srcCloseElement-1] != 1:
                    ...
                    while True:
                        try:
                            ...
                            telegram_bot.sendMessage(chat_id, "[BOT] : Erreur : Je n'arrive pas à close le LONG en cours, si ça se reproduit plus de 3 fois à la suite, veuillez me stopper, et allez couper le trade à la main.")
                            pass
                    message = "[BOT][Placement SL/TP LONG] : J'ai fermé le trade, la flèche à disparu. C'était un faut Trade."
                    while True:
                        try:
                            telegram_bot.sendMessage(chat_id, message)
                            break
                            ...
                            print(e)
                            pass
                elif DataCrypto['open'][srcCloseElement] >= TakeProfitPrice:
                    while True:
                        try:
                            positionInfo = client_binance.futures_position_information(symbol=pair, timestamp=int(time.time() * 1000))
                            break
                    ...
                    while True:
                        try:
                            ...
                            telegram_bot.sendMessage(chat_id, "[BOT] : Erreur : Je n'arrive pas à close le LONG en cours, si ça se reproduit plus de 3 fois à la suite, veuillez me stopper, et allez couper le trade à la main.")
                            pass
                    message = "[BOT][Placement SL/TP LONG] : J'ai fermé le trade, l'open était inférieur au StopLoss. Prix d'entrée : " + str(positionInfo[0].get('entryPrice')) + " Open : " + str(DataCrypto['open'][srcCloseElement]) + " StopLoss: " + str(StopLossPrice)
                    ...
                    while True:
                        try:
                            ...
                            telegram_bot.sendMessage(chat_id, "[BOT] : Erreur : Je n'arrive pas à récupérer la position du trade, si ça se reproduit plus de 3 fois à la suite, veuillez me stopper, et allez couper le trade à la main.")
                            pass
                    # Difference = float(positionInfo[0].get('entryPrice'))-float(DataCrypto['open'][srcCloseElement-1])
                    # if Difference > 0:
                    #     StopLossPrice += Difference
                    #     TakeProfitPrice += Difference 
                    while True:
                        try:
                ...
                if arrow[srcCloseElement-1] != -1:
                    ...
                    while True:
                        try:
                            ...
                            telegram_bot.sendMessage(chat_id, "[BOT] : Erreur : Je n'arrive pas à close le SHORT en cours, si ça se reproduit plus de 3 fois à la suite, veuillez me stopper, et allez couper le trade à la main.")
                            pass
                    message = "[BOT][Placement SL/TP SHORT] : J'ai fermé le trade, l'open était supérieur au StopLoss. Prix d'entrée : " + str(positionInfo[0].get('entryPrice')) + " Open : " + str(DataCrypto['open'][srcCloseElement]) + " StopLoss: " + str(StopLossPrice)
                    ...
                    while True:
                        try:
                            ...
                            pass
                    quan = float(positionInfo[0].get('positionAmt')) * -1.0
                    # Difference = float(DataCrypto['open'][srcCloseElement-1]) - float(positionInfo[0].get('entryPrice'))
                    # if Difference > 0:
                    #     StopLossPrice -= Difference
                    #     TakeProfitPrice -= Difference 
                    while True:
        ...
        if isLongTake == False and isShortTake == False and TradeDateStop != DataCrypto['date'][srcCloseElement]:
            ...
            if arrow[srcCloseElement] == 1 and color_bar[srcCloseElement] == 1 and lastRedID >= 0: #or color_bar[srcCloseElement] == 0):
                
                StopLossPrice = DataCrypto['close'][lastRedID]
                if float(DataCrypto['open'][srcCloseElement]) - ((float(DataCrypto['open'][srcCloseElement]) * 3.0)/100) <= float(DataCrypto['close'][lastRedID]) and StopLossPrice < DataCrypto['open'][srcCloseElement]:

                    StopLossPrice = round_decimals_down(StopLossPrice, int(pricePrecision))
                    TakeProfitPrice =  DataCrypto['open'][srcCloseElement] + (((DataCrypto['close'][lastRedID] - DataCrypto['open'][srcCloseElement]) * 1.5) * -1.0)
            ...
            elif arrow[srcCloseElement] == -1 and color_bar[srcCloseElement] == -1 and lastGreenID >= 0: #or color_bar[srcCloseElement] == 0):
                
                StopLossPrice = DataCrypto['close'][lastGreenID]
                if float(DataCrypto['open'][srcCloseElement]) + ((float(DataCrypto['open'][srcCloseElement]) * 3.0)/100) >= float(DataCrypto['close'][lastGreenID]) and StopLossPrice > DataCrypto['open'][srcCloseElement]:

                    StopLossPrice = round_decimals_down(StopLossPrice, int(pricePrecision))
                    TakeProfitPrice =  DataCrypto['open'][srcCloseElement] - (((DataCrypto['close'][lastGreenID] - DataCrypto['open'][srcCloseElement]) * 1.5))
                    TakeProfitPrice = round_decimals_down(TakeProfitPrice, int(pricePrecision));
                    while True:
                        try:
